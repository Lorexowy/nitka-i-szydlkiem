...
export class AuthService {
  ...
  static async updateUserProfile(uid: string, updates: Partial<UserProfile>): Promise<void> {
    try {
      ...
      // Jeśli aktualizujemy imię/nazwisko, zaktualizuj też displayName w users i w Firebase Auth
  ...
  static async getUserAddresses(uid: string): Promise<Address[]> {
    try {
      ...
      const q = query(addressesCol, where('userId', '==', uid))
  ...
  static async addUserAddress(
    ...
    try {
      ...
      if (addressData.isDefault) {
        const existing = await this.getUserAddresses(addressData.userId)
        const defaultOnes = existing.filter(a => a.isDefault)
        for (const a of defaultOnes) {
          await this.updateUserAddress(a.id, { isDefault: false })
        }
      }

      // Wariant 1: nowa referencja + setDoc (mamy id od razu)
      ...
      // Wariant 2 (alternatywa): addDoc
      // const addedRef = await addDoc(addressesCol, {
      //   ...addressData,
      //   createdAt: Timestamp.now(),
      //   updatedAt: Timestamp.now()
      // })
  ...
  static async setDefaultAddress(addressId: string, userId: string, type: 'shipping' | 'billing'): Promise<void> {
    try {
      ...
      const sameTypeDefaults = addresses.filter(a => a.type === type && a.isDefault)

      for (const a of sameTypeDefaults) {
        await this.updateUserAddress(a.id, { isDefault: false })
      }
